...
// Call ensureTestFiles before starting the server
ensureTestFiles();
...
app.post('/api/run-tests', async (req, res) => {
  ...
  try {
    console.log('Creating evaluator with config:', {
      ...config,
...
httpServer.listen(PORT, () => {
  ...
  // Try loading tests on startup to verify everything works
  try {
    const testCount = loadTests().length;
    console.log(`Successfully loaded ${testCount} tests on startup`);
  } catch (error) {
    console.error('Failed to load tests on startup:', error);
  }
}); 